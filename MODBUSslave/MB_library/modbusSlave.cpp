...
void modbusSlave::setBaud(word baud)
{
  ...
  //ModSerial.begin(baud); //configuration outside the class
...
*/
void modbusSlave::checkModSerial(void)
{
...
*/
void modbusSlave::ModSerialRx(void)
{
  byte i;
...
void modbusSlave::run(void)
{
  ...
  //retrieve the query message from the ModSerial uart
  this->ModSerialRx();
  ...
  {
    int i;
    //send the reply to the ModSerial UART
    //Senguino doesn't support a bulk ModSerial write command....
    for(i = 0 ; i < _len ; i++)
